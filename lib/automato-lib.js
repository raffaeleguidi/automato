...
function scpDeferredUpload(host, user, file, remote) {
...
function scpDeferredDownload(host, user, remoteFile, localDir) {
...
function uploadWithPromise(host, user, file, remote, callback) {
    
    scpDeferredUpload(host, user, file, remote)
     .then(
        function(res) {
            log.trace('exit code: ' + res.exitCode + ' uploading ' + file + ' to ' + host);
            res.host = host;
...
/* Versione con scp con promise */
function downloadWithPromise(host, user, remoteFile, localDir, callback) {
    
    scpDeferredDownload(host, user, remoteFile, localDir)
     .then(
        function(res) {
            log.trace('exit code: ' + res.exitCode + ' downloading ' + remoteFile + ' from ' + host);
            res.host = host;
...
function sshDeferredExec(host, user, command) {
...
function runWithPromise(host, user, command, callback) {
   
    sshDeferredExec(host, user, command)
     .then(
        function(res) {
            log.trace('executing ' + command + ' as user ' + user + ' on ' + host);
            log.trace(res.stderr);
